...
# Results
&nbsp;&nbsp;&nbsp;In order to perform a search using this interactive table, there are five filters a user can take into consideration in order to narrow down their search of UFO Sightings: Date, City, State, Country, State. These filters allow the user to interact with the table so that only the specified criteria appear. 
...
# Summary
&nbsp;&nbsp;&nbsp;Unfortunately, this web page does have a few drawbacks. Firstly, if a user inputs filters that result in no data entries, the table returns to the original dataset, returning to the full list of sightings. This may prove confusing to users. 
